...
% decoding 
fileID = fopen('input_decoder.mem','w');
for j = 1:number_codding_frame     
    for k = 1:Codeword_length
        fprintf(fileID,"%d",output_hadamard_reciever((j-1)*Codeword_length+k));
    end
    fprintf(fileID,"\n");
end
fclose(fileID);
...
for j = 1:number_codding_frame     
    for k = 1:Message_length
        fprintf(fileID,"%d",output_data((j-1)*Message_length+k));
    end
    fprintf(fileID,"\n");
end
fclose(fileID);
